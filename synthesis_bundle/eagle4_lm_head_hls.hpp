...
constexpr int kEagle4LmHiddenMax = 3072;
...
constexpr int kLmTcQpackFactor = tmac::hls::kI32WeightElems;   // qvalues per int32 (2b->16, 4b->8)
constexpr int kLmTcVocab       = 128256; // Llama-3.1-8B full vocab
constexpr int kLmMaxInPacks = (kEagle4LmRankMax + kLmTcQpackFactor - 1) / kLmTcQpackFactor;
constexpr int kLmMaxGroups = (kEagle4LmRankMax + kEagle4LmGroupSize - 1) / kEagle4LmGroupSize;
constexpr int kLmMaxVocabPacked = (kLmTcVocab + kLmTcQpackFactor - 1) / kLmTcQpackFactor;
constexpr int kLmDefaultZeroPoint = 1 << (tmac::hls::kQuantBits - 1);
...
void eagle4_lm_candidate_logits_row4(
    ...
    const int32_t qweight_row_major[kLmTcVocab * kLmMaxInPacks], // packed qvalues in int32
    const uint16_t scales_row_major[kLmTcVocab * kLmMaxGroups],    // expected [rank/group_size, vocab] (group-major)
    const int32_t qzeros_packed[kLmMaxVocabPacked * kLmMaxGroups], // expected [rank/group_size, ceil(vocab/kLmTcQpackFactor)] (group-major)
