...
constexpr int kE4dTransitionBatchSize = 1;
constexpr int kE4dTransitionNodeTopK = 8;
constexpr int kE4dTransitionHiddenSize = HIDDEN;
constexpr int kE4dTransitionMaxTreeWidth = TREE_WIDTH;
constexpr int kE4dTransitionMaxNodeCount = kHlsMaxNodeCount;
constexpr int kE4dTransitionMaxVerifyNum = kCdtFusedMaxBatch;
...
inline void cdt_run_eagle4_slm_topk_hls(
    ...
    int current_depth,
    const int* parent_indices_per_layer,
    float* reasoning_hidden_states_out,
    ...
    e4d_slm_topk(
        ...
        reasoning_hidden_states_out,
        topk_probas_sampling_out,
        topk_tokens_sampling_out);
}
...
// Transition entry for SGLang integration:
// - fixed batch size (1)
// - fixed node_top_k (8) and hidden size (HIDDEN)
// - fixed tree policy (no per-depth policy arrays; width/verify remain constant)
// Caller provides fixed_tree_width/fixed_verify_num for this invocation.
