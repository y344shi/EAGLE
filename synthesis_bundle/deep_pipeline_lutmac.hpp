...
#ifndef TMAC_BIND_PROJECTION_TILE_TO_BRAM
#define TMAC_BIND_PROJECTION_TILE_TO_BRAM 1
#endif

#ifndef TMAC_BIND_PROJECTION_WEIGHT_TILE_TO_URAM
#define TMAC_BIND_PROJECTION_WEIGHT_TILE_TO_URAM 1
#endif
...
#if TMAC_BIND_PROJECTION_TILE_TO_BRAM
#pragma HLS BIND_STORAGE variable=a_buffer type=ram_2p impl=bram
#if TMAC_BIND_PROJECTION_WEIGHT_TILE_TO_URAM
#pragma HLS BIND_STORAGE variable=weights_tile_bram type=ram_2p impl=uram
#else
...
#pragma HLS INTERFACE s_axilite port = return bundle = control
    ...
    constexpr int NUM_BANKS = E4D_PAR_W2B_NUM_BANKS;
    static_assert((NUM_BANKS & (NUM_BANKS - 1)) == 0,
                  "E4D_PAR_W2B_NUM_BANKS must be a power of two");
...
// Quantization: raw [0,3] → signed weight = raw - 2 ∈ {-2,-1,0,1}
// ============================================================

// Extract raw 2-bit weight at lane idx (0..255) from pack512. LSB-first.
...
template <int SCALE_EXP, int INPUT_DIM, int OUT_DIM = 256, int GROUP_SIZE = 128,
          bool ENABLE_TMAC = false>
...
#pragma HLS UNROLL factor=VEC_W
                    ...
                    if constexpr (ENABLE_TMAC) {
                        ...
                        prod = scale_pow2<SCALE_EXP>(a_scalar) *
                               static_cast<float>(decode_w2(w_raw)) * scale_val;
                    }
                    acc_banks[bank][lane] += prod;
                }
            }
...
#pragma HLS PIPELINE II=1
    ...
    float lut_pos[3];  // lut_pos[0]=-2a, [1]=-a, [2]=a  (raw=2 gives 0)
// #pragma HLS ARRAY_PARTITION variable=lut_pos complete
...
#pragma HLS loop_tripcount min=TILE max=TILE avg=TILE
#pragma HLS PIPELINE II=1
                scales_tile_bram[g * TILE + l] = scales[(g * TILES + t) * TILE + l];
            }
        }
w2b_compute_b:
        for (int b = 0; b < BATCH_SIZE; ++b) {
...
w2b_init_acc:
            for (int i = 0; i < TILE; ++i) {
#pragma HLS UNROLL factor=E4D_PAR_W2B_UNROLL
                for (int bi = 0; bi < NUM_BANKS; ++bi) {
#pragma HLS UNROLL
                    acc_banks[i][bi] = 0.0f;
                }
...
w2b_compute_k:
            for (int k = 0; k < INPUT_DIM; ++k) {
#pragma HLS loop_tripcount min=INPUT_DIM max=INPUT_DIM avg=INPUT_DIM
#pragma HLS PIPELINE II=E4D_PAR_W2B_COMPUTE_II
...
#pragma HLS UNROLL factor=E4D_PAR_W2B_UNROLL
                    ...
                    float prod;
                    if constexpr (ENABLE_TMAC) {
                        // raw=2 → w=0; raw=0→-2a, raw=1→-a, raw=3→+a
                        if (w_raw == 2) {
                            prod = 0.0f;
                        } else {
