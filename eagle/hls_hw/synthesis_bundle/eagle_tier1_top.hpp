...
#pragma HLS DATAFLOW
    ...
    constexpr int VECS_PER_Q = HEAD_DIM / VEC_W;                       // 8
    constexpr int VECS_PER_KV_TOKEN = (NUM_KV_HEADS * HEAD_DIM) / VEC_W; // 16
    constexpr int HEADS_PER_KV = NUM_HEADS / NUM_KV_HEADS;             // 16

    // Streams
    static hls_stream<vec_t<VEC_W>> s_in_attn;
    static hls_stream<vec_t<VEC_W>> s_in_resid;
    static hls_stream<vec_t<VEC_W>> s_norm;
    static hls_stream<vec_t<VEC_W>> s_q_in, s_k_in, s_v_in;
    static hls_stream<vec_t<VEC_W>> s_q_proj, s_k_proj, s_v_proj;
    static hls_stream<vec_t<VEC_W>> s_q_rot, s_k_rot;
    static hls_stream<vec_t<VEC_W>> s_k_hist_raw, s_v_hist_raw;
    static hls_stream<vec_t<VEC_W>> s_context;
    static hls_stream<vec_t<VEC_W>> s_o_proj, s_o_scaled;
    static hls_stream<vec_t<VEC_W>> s_res1, s_res1_norm_in, s_res1_skip;
    static hls_stream<vec_t<VEC_W>> s_ffn_norm, s_gate_in, s_up_in;
    static hls_stream<vec_t<VEC_W>> s_gate_vec, s_up_vec, s_swiglu, s_down;
...
#pragma HLS PIPELINE II = 1
    ...
    // Stage 14: Down projection (emit transformer block output; final residual is applied by caller)
    dense_projection_production_scaled<0, INTERMEDIATE, HIDDEN, 128, false>(s_swiglu, s_down, w_down, down_scales);
    stream_passthrough<VEC_W>(s_down, out_stream, HIDDEN / VEC_W);
}
