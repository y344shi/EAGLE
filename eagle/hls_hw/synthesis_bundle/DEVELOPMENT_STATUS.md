...
- **test_eagle_top.cpp**: Include-only compile sentinel that ensures the top-level header and shared utils stay self-consistent during refactors.
...
- **Top-level**: `eagle_tier1_top` compiles but attention is single-head; extending to all 32 heads and full FFN verification remains TODO. Connectivity/HBM bank assignments not yet defined.
...
- `eagle_tier1_top.hpp`: Dataflow skeleton for one token through the attention + FFN path (currently single-head attention). Stages: RMSNorm -> Q/K/V proj -> RoPE -> KV cache -> attention -> O proj -> residual (scaled) -> Norm2 -> gate/up projections -> SiLU*up -> down proj -> final residual. Uses packed weights/scales; TODO: fan out attention across 32 heads and add output verification.
...
- `test_eagle_top.cpp`: Minimal compile-check for including `eagle_tier1_top.hpp`; zero runtime logic, used as guard during refactors.
