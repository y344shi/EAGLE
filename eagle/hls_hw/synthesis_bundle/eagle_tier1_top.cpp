...
void e4_qdist(hls_stream<vec_t<VEC_W>>& s_q_rot, hls_stream<vec_t<VEC_W>> q_head_streams[TREE_WIDTH][NUM_HEADS]) {
...
#pragma HLS LOOP_TRIPCOUNT max = NUM_HEADS
        ...
        attn_sol<HEAD_DIM>(q_head_streams[t][h], k_head_streams[t][h], v_head_streams[t][h], ctx_head_streams[t][h], hist_len, padded_len);
#elif TMAC_ATTN_SOLVER_MODE == 1
        attn_pwl<HEAD_DIM>(q_head_streams[t][h], k_head_streams[t][h], v_head_streams[t][h], ctx_head_streams[t][h], hist_len, padded_len);
#else
...
void e4_ctx(hls_stream<vec_t<VEC_W>>& s_context, hls_stream<vec_t<VEC_W>> ctx_head_streams[TREE_WIDTH][NUM_HEADS]) {
#pragma HLS INLINE off
    for (int t = 0; t < TREE_WIDTH; t++) {
#pragma HLS loop_tripcount min=TREE_WIDTH max=TREE_WIDTH avg=TREE_WIDTH
    head_loop:
...
#pragma HLS DATAFLOW
    ...
    sdup<VEC_W>(hidden_in_stream, s_hidden_norm_in, s_hidden_residual, NUM_CHUNKS);

    // Stage 1: independent RMSNorm on hidden and embed branches
    rms_s<HIDDEN, TREE_WIDTH>(s_hidden_norm_in, s_hidden_norm, hidden_norm_gamma, RMS_EPS);
    rms_s<HIDDEN, TREE_WIDTH>(embed_in_stream, s_embed_norm, embed_norm_gamma, RMS_EPS);

    // Stage 2: layer-0 attention input concat [embed_norm || hidden_norm] (2H)
    e4_cat(s_embed_norm, s_hidden_norm, s_attn_cat);

    // Stage 3: triplicate for Q/K/V paths
    strip3<VEC_W>(s_attn_cat, s_q_in, s_k_in, s_v_in, NUM_CHUNKS * 2);

    // Stage 4: Q/K/V projections with 2H input
    dproj_b<0, TREE_WIDTH, QKV_INPUT, HIDDEN, 128, TMAC_USE_TMAC_QKV>(s_q_in, s_q_proj, w_q, s_q);
    dproj_b<0, TREE_WIDTH, QKV_INPUT, NUM_KV_HEADS * HEAD_DIM, 128, TMAC_USE_TMAC_QKV>(s_k_in, s_k_proj, w_k, s_k);
    dproj_b<0, TREE_WIDTH, QKV_INPUT, NUM_KV_HEADS * HEAD_DIM, 128, TMAC_USE_TMAC_QKV>(s_v_in, s_v_proj, w_v, s_v);

    // Stage 5: RoPE on Q/K
    rope_s<NUM_HEADS, NUM_KV_HEADS, HEAD_DIM, TREE_WIDTH>(s_q_proj, s_q_rot, s_k_proj, s_k_rot, rope_cfg);

    // Stage 6: Write new KV to contiguous HBM, then gather prefix + ancestors + self.
    ckv_wg<HEAD_DIM, NUM_KV_HEADS, kMaxDraftDepth>(
    ...
    e4_qdist(s_q_rot, q_head_streams);
    e4_kvbcast(s_k_hist_raw, s_v_hist_raw, k_head_streams, v_head_streams, hist_len);
    e4_gqa(q_head_streams, k_head_streams, v_head_streams, ctx_head_streams, hist_len, padded_len);
    e4_ctx(s_context, ctx_head_streams);

    // Stage 8: output projection
    dproj_b<0, TREE_WIDTH, HIDDEN, HIDDEN, 128, TMAC_USE_TMAC_O>(s_context, s_o_proj, w_o, s_o);

    // Stage 9: post-attn residual + post-attn RMSNorm (returns both residual and normalized stream)
    sadd<VEC_W>(s_o_proj, s_hidden_residual, s_post_attn_residual, NUM_CHUNKS);
    sdup<VEC_W>(s_post_attn_residual, s_post_attn_norm_in, s_post_attn_residual_for_add, NUM_CHUNKS);
    rms_s<HIDDEN, TREE_WIDTH>(s_post_attn_norm_in, s_post_attn_norm, post_attn_norm_gamma, RMS_EPS);

    // Stage 10: FFN gate/up + SiLU
    sdup<VEC_W>(s_post_attn_norm, s_gate_in, s_up_in, NUM_CHUNKS);
    dproj_b<0, TREE_WIDTH, HIDDEN, INTERMEDIATE, 128, TMAC_USE_TMAC_FFN>(s_gate_in, s_gate_vec, w_gate, gate_scales);
    dproj_b<0, TREE_WIDTH, HIDDEN, INTERMEDIATE, 128, TMAC_USE_TMAC_FFN>(s_up_in, s_up_vec, w_up, up_scales);
    swiglu<VEC_W>(s_gate_vec, s_up_vec, s_swiglu, (TREE_WIDTH * INTERMEDIATE) / VEC_W);

    // Stage 11: down-proj to 2H and split (to_logits, for_reasoning)
    dproj_b<0, TREE_WIDTH, INTERMEDIATE, DOWN_OUTPUT, 128, TMAC_USE_TMAC_FFN>(s_swiglu, s_down_2hs, w_down, down_scales);
    e4_split(s_down_2hs, s_to_logits_raw, s_for_reasoning);

    // Stage 12: final norm on logits stream and residual add on reasoning stream
    rms_s<HIDDEN, TREE_WIDTH>(s_to_logits_raw, logits_norm_out_stream, final_norm_gamma, RMS_EPS);
    sadd<VEC_W>(s_for_reasoning, s_post_attn_residual_for_add, reasoning_out_stream, NUM_CHUNKS);
}
