...
void eagle_tier1_lm_top(
    hls::stream<tmac::hls::vec_t<tmac::hls::VEC_W>>& in_stream,
    int* best_id,
    float* best_score,
    ...
    // 2. Collect into LM head buffer (Output Expansion)
    ...
    // Loop for PIPELINED writing
    convert_loop: for (int i = 0; i < tmac::hls::HIDDEN / tmac::hls::VEC_W; ++i) {
        #pragma HLS PIPELINE II=1
        auto v = block_out.read();
        
        expand_vec: for (int j = 0; j < tmac::hls::VEC_W; ++j) {
            #pragma HLS UNROLL
            int k = i * tmac::hls::VEC_W + j;
            lm_input_buffer[k * T_BATCH + 0] = (dtype_in)v[j];
            
            // Pipelined zero filling
            fill_zeros: for (int t = 1; t < T_BATCH; ++t) {
